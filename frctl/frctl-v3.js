...
function advancedialog(event) {
    ...
    if (pagenum >= ALLDIALOG[dname].length) {
        ...
        enddialog()
        if (dname == "startup") {
            window.localStorage.setItem("introseen-prd", true)
        }
        return
    }
...
ready(async () => {
    try {
        ALLSLUGS = await (await fetch("./slugs-v4.json")).json()
        ALLDIALOG = await (await fetch("./dialog-prd.json")).json()
    } catch {
    ...
    const introSeen = window.localStorage.getItem("introseen-prd")
    if (!introSeen) {
        advancedialog()
    } else {
        enddialog()
