...
easy = [carddata[cn]["slug"] for cn,cs in b.cards][:100]

# Medium: at least 1k appearances but not in Easy
med = [carddata[cn]["slug"] for cn,cs in b.cards if cs.num_appearances >= 1000][100:]
...
hard = [carddata[cn]["slug"] for cn,cs in b.cards if cs.num_appearances >= 100]
hard = [slug for slug in hard if slug not in med and slug not in easy]
hard += SELECTED_TOKENS

# Maniac: Everything else
maniac = [carddata[cn]["slug"] for cn,cs in b.cards if cs.num_appearances < 100 and carddata[cn]["slug"] not in SELECTED_TOKENS]
